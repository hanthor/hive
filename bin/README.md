...
| `hive-checkout-guard.sh` | Deploy | `ExecStartPre` guard for systemd units that execute code out of a checkout (e.g. `hive-discord.service`, `hive-snapshot.service`): refuses to let the unit start when the checkout directory or its entrypoint files could have been planted by an unprivileged local user (#5435). Its exit status is the assertion — every unsafe path exits non-zero, so a violation actually stops `ExecStart`. |
...
| `test_enumerate_actionable.sh` | Contract tests for `enumerate-actionable.sh`, which writes `/var/run/hive-metrics/actionable.json` — the one file `merge-gate.sh`, `kick-agents.sh`, and the dashboard's contribute lane all consume. |
| `test_merge_gate.sh` | Contract tests for `merge-gate.sh`: how `actionable.json` is turned into `/var/run/hive-metrics/merge-eligible.json`. |
| `test_kick_governor.sh` | Contract tests for `kick-governor.sh`, the adaptive kick timer that ladders scanner/ci-maintainer/architect/outreach cadence from the actionable backlog. |
| `test_gh_rate_check.sh` | Contract tests for `gh-rate-check.sh`: GitHub API rate-limit detection in agent tmux panes and the pause/pullback decisions it makes. |
| `test_backend_smoke.sh` | End-to-end smoke for the contributor-agent backends: exercises the seam with vendor coding-agent CLIs (completion detection, flags, output shape) that vendors can break on their own release schedule (#5376). |
| `test_backend_smoke_attribution.sh` | backend-smoke's failure-issue script must attribute failures truthfully (#5883): an infrastructure failure (e.g. missing podman on the fleet image) must not be reported as a CLI-version regression. |
| `backend_smoke_attribution_harness.js` | Harness used by `test_backend_smoke_attribution.sh`: executes `backend-smoke.yml`'s failure-issue script against fixtures so its attribution is verified by running it rather than by reading it (#5883). |
| `test_dibs_cutover_preflight.sh` | Contract tests for `src/deploy/dibs-domain-cutover/preflight.sh` (#5925): every case executes the preflight against stub `kubectl`/`dig`/`curl` and asserts on exit code and report — no cluster, no network. |
| `test_dibs_cutover_verify.sh` | Contract tests for `src/deploy/dibs-domain-cutover/verify.sh` (#5925): every case executes the verifier against stub `openssl`/`curl`/`dig`/`kubectl` and asserts on exit code and report. Pins that each silent-pass failure mode — a self-signed certificate, a certificate missing the new SAN, an inert redirect, a redirect that drops the path and query, a hub on a foreign registrable domain — blocks rather than reading as a finished cutover. No cluster, no network. |
