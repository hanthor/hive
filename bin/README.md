...
| `hive-checkout-guard.sh` | Deploy | `ExecStartPre` guard installed by `hive-deploy.sh` for systemd units (`hive-discord.service`, `hive-snapshot.service`) that execute code out of `/tmp/hive`: refuses to start unless every directory from `/` down to the checkout is owned by the service user or root and not writable by anyone else, and the entrypoint file is a non-symlink, non-group/world-writable regular file (#5435). |
...
| `test_enumerate_actionable.sh` | Contract tests for `enumerate-actionable.sh`: the hold/blocked/draft/ADOPTERS-only/external-issue-without-SHA exclusions that produce `/var/run/hive-metrics/actionable.json`, the one file `merge-gate.sh`, `kick-agents.sh`, and the dashboard's contribute lane read instead of running their own `gh` queries. |
| `test_merge_gate.sh` | Contract tests for `merge-gate.sh`: the hold/CI/mergeable/author gates that decide whether a PR is written into `/var/run/hive-metrics/merge-eligible.json` as safe to merge. |
| `test_kick_governor.sh` | Contract tests for `kick-governor.sh`: the surge/busy/quiet/idle cadence ladder, token-budget model downgrades, and which agent gets kicked or paused each tick. |
| `test_gh_rate_check.sh` | Contract tests for `gh-rate-check.sh`: the tmux-pane rate-limit patterns that decide which agents get paused, and for how long, during a real GitHub API rate limit. |
